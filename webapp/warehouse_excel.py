...
def build_tx_template() -> bytes:
    ...
    samples = [
        ...
        ["V-002", today, "منصرف للمعاملة", "M-001", "كيبل 4×16", "متر", 25, "فرقة 1", "T-100", "خريص", "ربط بالمعاملة/العطل"],
    ]
    for offset, sample in enumerate(samples):
        r = header_row + 1 + offset
        for col, val in enumerate(sample, start=1):
    ...
    brand.write_instructions_sheet(
        wb,
        [
            "أنواع الحركة المقترحة: وارد من الكهرباء | منصرف للمعاملة | إرجاع للمجمعة | وارد من موقع العمل | رصيد افتتاحي",
